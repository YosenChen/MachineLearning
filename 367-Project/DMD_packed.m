...
%%
%SOLVE " minimize{I_DM} ||AI_DM-b||_2^2 + NLM 
...
for c=1:3
    ...
    for i=1:niter
        ...
        %update x
        v = z-u;
        Atilde=@(x) reshape(AtfunDM(c,Afun(c,pat,reshape(x,[m n])))+p.*reshape(x,[m n]),[m*n 1]); 
        btilde = reshape(AtfunDM(c,b(:,:,c)) + p.*v,[m*n 1]);

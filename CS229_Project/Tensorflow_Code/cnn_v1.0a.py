...
if BATCH_SIZE > 2*TRAIN_TEST_ALLOC:
    BATCH_SIZE = 2*TRAIN_TEST_ALLOC
...
kaishu_train_label = np.array([[1,0]])[[0]*len(kaishu_train)]
caoshu_train_label = np.array([[0,1]])[[0]*len(caoshu_train)]
...
kaishu_test_label = np.array([[1,0]])[[0]*len(kaishu_test)]
caoshu_test_label = np.array([[0,1]])[[0]*len(caoshu_test)]
...
train_x = np.concatenate((kaishu_train, caoshu_train), axis=0)
train_y = np.concatenate((kaishu_train_label, caoshu_train_label), axis=0)

test_x = np.concatenate((kaishu_test, caoshu_test), axis=0)
test_y = np.concatenate((kaishu_test_label, caoshu_test_label), axis=0)
...
W = tf.Variable(tf.zeros([PIX**2, 2]))
...
for iter in range(CNN_MAX_ITERATION):
	if BATCH_SIZE == 2*TRAIN_TEST_ALLOC:
	    batch_ind = range(2*TRAIN_TEST_ALLOC) 
	else:

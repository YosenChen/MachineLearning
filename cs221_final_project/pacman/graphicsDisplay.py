...
# Capsule graphics
CAPSULE_COLOR = formatColor(.9,0,0)
CAPSULE_SIZE = 0.4
...
class InfoPane:
  ...
  def drawPane(self):
    self.scoreText = text( self.toScreen(0, 0  ), self.textColor, "ENERGY LOSS:    0", "Times", self.fontSize, "bold")
  ...
  def updateScore(self, score):
    changeText(self.scoreText, "ENERGY LOSS: % 4d" % score)
...
class PacmanGraphics:
  ...
  def update(self, newState):
    ...
    if agentState.isPacman:
      ...
      #ModifiedByKai
      #Ghost can have the same animation as Pacman does
      self.animatePacman(agentState, prevState, prevImage)
      # self.moveGhost(agentState, agentIndex, prevState, prevImage)
    self.agentImages[agentIndex] = (agentState, prevImage)
    if newState._foodEaten != None:
      for foodEaten in newState._foodEaten:
        self.removeFood(foodEaten, self.food)
